...
namespace PPL.Omex.Bot.Tests.Dialogs.Payment
{
    ...
    public class ReconnectTests : OmexDialogTestBase
    {
        ...
        private const string ExpectedPowerWillBeOnSoonMessage = "Your services will be turned back on within the next 24 hours."; // Adjusted based on AC1
        ...
        private const string PlaceholderEndOfStory = "You reached gas reconnect phone entry placeHolder."; // This is from your existing code, implies entry to StopPhoneEntry dialog

        // IVR Prompts assumed from AC1 and StopPhoneEntry.cs
        private const string ExpectedPhoneNumberEntryPrompt = "Please say or enter the phone number of the adult who will be at the reconnect services address";
        private const string ExpectedPhoneNumberHeardPromptPrefix = "We heard "; // Followed by [phone number]
        private const string ExpectedPhoneNumberConfirmationPrompt = "Is that correct?";
        ...
        private static class FlowDataGenerator
        {
            public static IEnumerable<object[]> TestCases()
            {
                ...
                // Original test case for Gas Service - updated below to simulate full AC1 flow
                // Test Case 14 (previously Gas Service, Cut Date >= today)
                // Renamed to Reconnection_NotEligible_CutInOrderIssued_CutInDate_IsToday_Gas_Original
                testCaseData = new FlowData()
                {
                    Test = "Reconnection_NotEligible_CutInOrderIssued_CutInDate_IsToday_Gas_Original",
                    AccountId = 123456789,
                    IsEligible = false,
                    HasCutInOrderIssued = true,
                    CutInOrderDate = DateTime.Now.Date,
                ...
                testCaseData.Turns = new Turn[]
                {
                        new Turn { N = "Cut-In Order Issued Cut-In Date Is Today", U = null, Rs =
                            new List<string>
                            {
                                ...
                                PlaceholderEndOfStory, // Entry into StopPhoneEntry dialog
                                ExpectedBackToFlow // This was the original expected result, now expanded below for full AC1
                            }
                        }
                };
                yield return new object[] { new TestDataObject(testCaseData) };
                ...
                // NEW TEST CASE FOR FULL AC1 FLOW
                testCaseData = new FlowData()
                {
                    Test = "Reconnection_AC1_FullFlow_GasCustomer",
                    AccountId = 123456789,
                    IsEligible = false,
                    HasCutInOrderIssued = true,
                    CutInOrderDate = DateTime.Now.Date.AddDays(1), // Using future date as per AC1's "NOT TODAY" interpretation
                    IsGasAccount = true
                };
                testCaseData.Turns = new Turn[]
                {
                    // 1. Initial prompts from Reconnect dialog (based on AC1)
                    new Turn { N = "AC1 - Prompt 1", U = null, R = ExpectedPowerWillBeOnSoonMessage },
                    new Turn { N = "AC1 - Prompt 2", U = null, R = ExpectedGasAdultPhoneMessage },

                    // 2. Routing to Stop Service Phone Entry Flow, and its first prompt (from AC1)
                    new Turn { N = "AC1 - Phone Entry Prompt", U = null, R = ExpectedPhoneNumberEntryPrompt }, // "Please say or enter the phone number..."

                    // 3. User says a valid number
                    new Turn { N = "AC1 - User Says Number", U = "5551234567", R = $"{ExpectedPhoneNumberHeardPromptPrefix}5551234567" }, // "We heard [phone number]"
                    
                    // 4. System asks for confirmation
                    new Turn { N = "AC1 - Confirmation Prompt", U = null, R = ExpectedPhoneNumberConfirmationPrompt }, // "Is that correct?"

                    // 5. User says "YES"
                    new Turn { N = "AC1 - User Confirms", U = "yes", R = ExpectedBackToFlow } // Child dialog ends, then parent dialog ends as per "Then the call Ends"
                };
                yield return new object[] { new TestDataObject(testCaseData) };
